...
public:
	...
	Class_T Classify( void )	{	return CLASS_HOUNDEYE;	}	//FIXME: No classification for various wildlife?
...
// Silence classcheck
//	DEFINE_FIELD( CNPC_Ichthyosaur, m_LastSteer, FIELD_VECTOR ),

	DEFINE_FIELD( CNPC_Ichthyosaur, m_pVictim,				FIELD_CLASSPTR ),
	DEFINE_FIELD( CNPC_Ichthyosaur, m_vecLastMoveTarget,	FIELD_VECTOR ),
	DEFINE_FIELD( CNPC_Ichthyosaur, m_flNextBiteTime,		FIELD_FLOAT ),
	DEFINE_FIELD( CNPC_Ichthyosaur, m_flHoldTime,			FIELD_FLOAT ),
	DEFINE_FIELD( CNPC_Ichthyosaur, m_flSwimSpeed,			FIELD_FLOAT ),
	DEFINE_FIELD( CNPC_Ichthyosaur, m_flTailYaw,			FIELD_FLOAT ),
	DEFINE_FIELD( CNPC_Ichthyosaur, m_flTailPitch,			FIELD_FLOAT ),
	DEFINE_FIELD( CNPC_Ichthyosaur, m_flNextPingTime,		FIELD_FLOAT ),
	DEFINE_FIELD( CNPC_Ichthyosaur, m_flNextGrowlTime,		FIELD_FLOAT ),
	DEFINE_FIELD( CNPC_Ichthyosaur, m_bHasMoveTarget,		FIELD_BOOLEAN ),
	DEFINE_FIELD( CNPC_Ichthyosaur, m_bIgnoreSurface,		FIELD_BOOLEAN ),

	//DEFINE_FUNCTION( CNPC_Ichthyosaur, IchTouch ),
...
//	m_vecCurrentVelocity	= forward * m_flGroundSpeed;
	...
	//m_pSwimSound	= ENVELOPE_CONTROLLER.SoundCreate( edict(), CHAN_BODY,	"npc/ichthyosaur/ich_amb1.wav", ATTN_NORM );
	//m_pVoiceSound	= ENVELOPE_CONTROLLER.SoundCreate( edict(), CHAN_VOICE,	"npc/ichthyosaur/water_breath.wav", ATTN_IDLE );
...
bool CNPC_Ichthyosaur::FVisible( CBaseEntity *pEntity, int traceMask, CBaseEntity **ppBlocker )
{
	...
	trace_t tr;
	AI_TraceLine(vecLookerOrigin, vecTargetOrigin, MASK_OPAQUE, this, COLLISION_GROUP_NONE, &tr);
...
void CNPC_Ichthyosaur::EnsnareVictim( CBaseEntity *pVictim )
{
	...
	if ( pBCC && pBCC->HandleInteraction( g_interactionBarnacleVictimGrab, NULL, this ) )
	{
		if ( pVictim->IsPlayer() )
		{
...
void CNPC_Ichthyosaur::ReleaseVictim( void )
{
	...
	pBCC->HandleInteraction( g_interactionBarnacleVictimReleased, NULL, this );

	m_pVictim->RemoveSolidFlags( FSOLID_NOT_SOLID );
	UTIL_Relink( m_pVictim );
...
//			&tolerance - 
//-----------------------------------------------------------------------------
void CNPC_Ichthyosaur::TranslateEnemyChasePosition( CBaseEntity *pEnemy, Vector &chasePosition, float &tolerance )
{
	Vector offset = pEnemy->EyePosition() - pEnemy->GetAbsOrigin();
	chasePosition += offset;
	tolerance = GetHullWidth()*2.0f;
}
