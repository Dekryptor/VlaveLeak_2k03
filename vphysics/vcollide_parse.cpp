...
void CVPhysicsParse::ParseFluid( fluid_t *pFluid, IVPhysicsKeyHandler *unknownKeyHandler )
{
	...
	while ( m_pText )
	{
		...
		{
			pFluid->params.density = atof( value );
		}
		else if ( !Q_stricmp( key, "damping" ) )
		{
...
void CVPhysicsParse::ParseVehicleSteering( vehicle_steeringparams_t &steering )
{
	...
	while ( m_pText )
	{
		...
		if ( key[0] == '}' )
			return;
		// parse subchunks
		if ( !Q_stricmp( key, "degrees" ) )
		{
			steering.degrees = atof( value );
		}
		...
		else if ( !Q_stricmp( key, "steeringRestFactor" ) )
		{
			steering.steeringRestFactor = atof( value );
		}
