...
// Output : CChoreoChannel
//-----------------------------------------------------------------------------
int CChoreoScene::GetChannelIndex( CChoreoChannel *pChannel )
{
	for (int channel = 0; channel < GetNumChannels(); channel++ )
	{
		if (pChannel == GetChannel( channel ))
		{
			return channel;
		}
	}
...
void CChoreoScene::ParseFlexAnimations( ISceneTokenProcessor *tokenizer, CChoreoEvent *e, bool removeold /*= true*/ )
{
	...
	if ( !_stricmp( tokenizer->CurrentToken(), "samples_use_time" ) )
	{
	...
	while ( 1 )
	{
		...
		strcpy( flexcontroller, tokenizer->CurrentToken() );
		...
		if ( !_stricmp( tokenizer->CurrentToken(), "disabled" ) )
		{
			active = false;
			tokenizer->GetToken( true );
		}
		
		if ( !_stricmp( tokenizer->CurrentToken(), "combo" ) )
		{
			combo = true;
			tokenizer->GetToken( true );
		}
		
		if ( !_stricmp( tokenizer->CurrentToken(), "range" ) )
		{
			tokenizer->GetToken( false );
...
// Output : CChoreoEvent
//-----------------------------------------------------------------------------
CChoreoEvent *CChoreoScene::ParseEvent( CChoreoActor *actor, CChoreoChannel *channel, float timeoffset /*= 0.0f*/ )
{
	...
	while ( 1 )
	{
		...
		if ( !_stricmp( m_pTokenizer->CurrentToken(), "time" ) )
		{
		...
		else if ( !_stricmp( m_pTokenizer->CurrentToken(), "ramp" ) )
		{
			hadramp = true;
		...
		else if ( !_stricmp( m_pTokenizer->CurrentToken(), "param" ) )
		{
			m_pTokenizer->GetToken( false );

			e->SetParameters( m_pTokenizer->CurrentToken() );
		}
		else if ( !_stricmp( m_pTokenizer->CurrentToken(), "param2" ) )
		{
			m_pTokenizer->GetToken( false );

			e->SetParameters2( m_pTokenizer->CurrentToken() );
		}
		else if ( !_stricmp( m_pTokenizer->CurrentToken(), "pitch" ) )
		{
			m_pTokenizer->GetToken( false );
			e->SetPitch( atoi( m_pTokenizer->CurrentToken() )  );
		}
		else if ( !_stricmp( m_pTokenizer->CurrentToken(), "yaw" ) )
		{
			m_pTokenizer->GetToken( false );
			e->SetYaw( atoi( m_pTokenizer->CurrentToken() )  );
		}
		else if ( !_stricmp( m_pTokenizer->CurrentToken(), "loopcount" ) )
		{
			m_pTokenizer->GetToken( false );
			e->SetLoopCount( atoi( m_pTokenizer->CurrentToken() ) );
		}
		else if ( !_stricmp( m_pTokenizer->CurrentToken(), "resumecondition" ) )
		{
			e->SetResumeCondition( true );
		}
		else if ( !_stricmp( m_pTokenizer->CurrentToken(), "fixedlength" ) )
		{
			e->SetFixedLength( true );
		}
		else if ( !_stricmp( m_pTokenizer->CurrentToken(), "tags" ) )
		{
			// Parse tags between { }
			//
			m_pTokenizer->GetToken( true );
			...
			while ( 1 )
			{
				...
				strcpy( tagname, m_pTokenizer->CurrentToken() );
				m_pTokenizer->GetToken( false );
		...
		else if ( !_stricmp( m_pTokenizer->CurrentToken(), "sequenceduration" ) )
		{
			float duration = 0.0f;
		...
		else if ( !_stricmp( m_pTokenizer->CurrentToken(), "absolutetags" ) )
		{
			m_pTokenizer->GetToken( true );
		...
		else if ( !_stricmp( m_pTokenizer->CurrentToken(), "flextimingtags" ) )
		{
			// Parse tags between { }
			//
			m_pTokenizer->GetToken( true );
		...
		else if ( !_stricmp( m_pTokenizer->CurrentToken(), "relativetag" ) )
		{
			...
			strcpy( tagname, m_pTokenizer->CurrentToken() );
			m_pTokenizer->GetToken( false );
			strcpy( wavname, m_pTokenizer->CurrentToken() );

			e->SetUsingRelativeTag( true, tagname, wavname );
		}
		else if ( !_stricmp( m_pTokenizer->CurrentToken(), "flexanimations" ) )
		{
			ParseFlexAnimations( m_pTokenizer, e );
		}
		else if ( !_stricmp( m_pTokenizer->CurrentToken(), "event_ramp" ) )
		{
...
CChoreoActor *CChoreoScene::ParseActor( void )
{
	...
	while ( 1 )
	{
		...
		if ( !_stricmp( m_pTokenizer->CurrentToken(), "channel" ) )
		{
			ParseChannel( a );
		}
		else if ( !_stricmp( m_pTokenizer->CurrentToken(), "faceposermodel" ) )
		{
			ParseFacePoserModel( a );
		}
		else if ( !_stricmp( m_pTokenizer->CurrentToken(), "active" ) )
		{
			m_pTokenizer->GetToken( true );
...
void CChoreoScene::ParseMapname( void )
{
	...
	m_pTokenizer->GetToken( true );
	strcpy( m_szMapname, m_pTokenizer->CurrentToken() );
}
...
{
	m_pTokenizer->GetToken( true );
	m_bUseFrameSnap = !_stricmp( m_pTokenizer->CurrentToken(), "on" ) ? true : false;
}
...
CChoreoChannel *CChoreoScene::ParseChannel( CChoreoActor *actor )
{
	...
	while ( 1 )
	{
		...
		if ( !_stricmp( m_pTokenizer->CurrentToken(), "event" ) )
		{
			ParseEvent( actor, c );
		}
		else if ( !_stricmp( m_pTokenizer->CurrentToken(), "active" ) )
...
//-----------------------------------------------------------------------------
bool CChoreoScene::ParseFromBuffer( ISceneTokenProcessor *tokenizer )
{
	m_pTokenizer = tokenizer;

	// xxxTODO Check version
	/*
	...
	*/
	...
	while ( 1 )
	{
		...
		else if ( !_stricmp( m_pTokenizer->CurrentToken(), "actor" ) )
		{
			ParseActor();
		}
		else if ( !_stricmp( m_pTokenizer->CurrentToken(), "mapname" ) )
		{
			ParseMapname();
		}
		else if ( !_stricmp( m_pTokenizer->CurrentToken(), "fps" ) )
		{
			ParseFPS();
		}
		else if ( !_stricmp( m_pTokenizer->CurrentToken(), "snap" ) )
		{
			ParseSnap();
		}
		else if ( !_stricmp( m_pTokenizer->CurrentToken(), "scene_ramp" ) )
		{
			ParseSceneRamp( m_pTokenizer, this );
		}
		else if ( !_stricmp( m_pTokenizer->CurrentToken(), "scalesettings" ) )
		{
...
//			*e - 
//-----------------------------------------------------------------------------
void CChoreoScene::FileSaveEvent( CUtlBuffer& buf, int level, CChoreoEvent *e, float starttimeoffset /* = 0.0f */ )
{
...
bool CChoreoScene::CheckEventCompletion( void )
{
	...
	// Put all items into the pending queue
	for ( int i = 0; i < m_ResumeConditions.Size(); i++ )
	{
		e = m_ResumeConditions[ i ];
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CChoreoScene::FindPauseBetweenTimes( float starttime, float endtime )
{
	CChoreoEvent *e;
	...
	case CChoreoEvent::SPEAK:
		...
		//  current latency
		{
			if ( playing_forward )
			{
				...
				//  adjusted start time
				if ( FindPauseBetweenTimes( starttime, starttime + m_flSoundSystemLatency ) )
				{
					suppressed = true;
				}
			}
			else // VXP: FIXME: Can comment this leaf to prevent playing backwards
			{
				...
				// Search for pause condition in between the original time and the
				//  adjusted start time
				if ( FindPauseBetweenTimes( endtime - m_flSoundSystemLatency, endtime ) )
				{
					suppressed = true;
				}
			}
...
void CChoreoScene::SwapActors( int a1, int a2 )
{
	...
	m_Actors.Remove( a1 );
	m_Actors.InsertBefore( a1, pactor2 );
	m_Actors.Remove( a2 );
	m_Actors.InsertBefore( a2, pactor1 );
}
	...
	for ( int i = 0 ; i < m_Actors.Size(); i++ )
	{
		...
		for ( int j = 0; j < a->GetNumChannels(); j++ )
		{
			...
			for ( int k = 0 ; k < c->GetNumEvents(); k++ )
			{
				...
				if ( _stricmp( wavname, e->GetRelativeWavName() ) )
					continue;

				if ( _stricmp( name, e->GetRelativeTagName() ) )
					continue;
...
void CChoreoScene::ExportEvents( const char *filename, CUtlVector< CChoreoEvent * >& events )
{
	...
	// Look for selected events that have actor/channel set
	int i;
	for ( i = 0 ; i < events.Size(); i++ )
	{
		...
		if ( i == 0 || ( e->GetStartTime() < earlieststart ) )
		{
			earlieststart = e->GetStartTime();
		}
	}
	...
	for ( i = 0 ; i < events.Size(); i++ )
	{
		...
		FileSaveEvent( buf, 0, e, earlieststart );
	}
...
//			starttime - 
//-----------------------------------------------------------------------------
void CChoreoScene::ImportEvents( ISceneTokenProcessor *tokenizer, CChoreoActor *actor, CChoreoChannel *channel, float starttime )
{
	m_pTokenizer = tokenizer;
	...
	while ( 1 )
	{
		...
		if ( !_stricmp( m_pTokenizer->CurrentToken(), "event" ) )
		{
			ParseEvent( actor, channel, starttime );
		}
		else
		{
			m_pTokenizer->Error( "unexpected token %s\n", m_pTokenizer->CurrentToken() );
