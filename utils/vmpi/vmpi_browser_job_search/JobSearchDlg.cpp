...
void CJobSearchDlg::OnDblclkJobsList(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if ( iItem != -1 )
	{
		...
		CString cmdLine;
		cmdLine.Format( "vmpi_browser_job_watch -JobID %d -dbname \"%s\" -hostname \"%s\" -username \"%s\"", 
		...
		{
			CString errStr;
			errStr.Format( "Error launching '%s'", cmdLine );
...
void CJobSearchDlg::RepopulateJobsList()
{
	...
	while ( GetMySQL()->NextRow() )
	{
		...
		pInfo->m_JobID = GetMySQL()->GetColumnValue( "JobID" ).UInt32();
		...
		pInfo->m_RunningTimeMS = GetMySQL()->GetColumnValue( "RunningTimeMS" ).UInt32();
...
void CJobSearchDlg::OnDblclkUserList() 
{
	...
	if ( sel != LB_ERR )
	{
		...
		CMySQLQuery query;
		query.Format( "select RunningTimeMS, JobID, BSPFilename, StartTime, MachineName from job_master_start where MachineName=\"%s\"", computerName );
...
void CJobSearchDlg::OnDblclkWorkerList() 
{
	...
	if ( sel != LB_ERR )
	{
		...
		CMySQLQuery query;
		query.Format( 
			"select RunningTimeMS, job_master_start.JobID, BSPFilename, StartTime, job_master_start.MachineName "
...
BOOL CJobSearchDlg::OnInitDialog() 
{
	...
	m_pSQL = InitMySQL( m_DBName, m_HostName, m_UserName );
	if ( !m_pSQL )
	{
		Q_snprintf( str, sizeof( str ), "Can't init MYSQL db (db = '%s', host = '%s', user = '%s')", m_DBName, m_HostName, m_UserName );
		MessageBox( str, "Error", MB_OK );
		EndDialog( 0 );
		return FALSE;
	}
...
void CJobSearchDlg::PopulateWorkerList()
{
	m_WorkerList.ResetContent();

	GetMySQL()->Execute( "select MachineName from job_worker_start group by MachineName" );
	while ( GetMySQL()->NextRow() )
	{
		m_WorkerList.AddString( GetMySQL()->GetColumnValue( "MachineName" ).String() );
	}
}

void CJobSearchDlg::PopulateUserList()
{
	m_UserList.ResetContent();

	GetMySQL()->Execute( "select MachineName from job_master_start group by MachineName" );
	while ( GetMySQL()->NextRow() )
	{
		m_UserList.AddString( GetMySQL()->GetColumnValue( "MachineName" ).String() );
	}
}
